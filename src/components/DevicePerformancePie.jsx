...
function DevicePerformancePie({ activeCampaign, activeProperty, period, customDates, isAnalytics = false }) {
  ...
  const apiCallFunction = isAnalytics
    ? async (propertyId, analyticsPeriod, customDatesParam) => {
        ...
        let url = `${import.meta.env.VITE_API_BASE_URL}/api/analytics/audience-insights/${propertyId}?dimension=deviceCategory&period=${analyticsPeriod}`;
    ...
    : async (customerId, adsPeriod, customDatesParam) => {
        ...
        let url = `${import.meta.env.VITE_API_BASE_URL}/api/ads/device-performance/${customerId}?period=${adsPeriod}`;
